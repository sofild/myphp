...
class UploadHandler
{
    ...
    function __construct($options = null, $initialize = true) {
        $this->options = array(
            'script_url' => $this->get_full_url().'/extends/upload/server/php/',
            'upload_dir' => dirname(dirname(dirname(dirname(dirname($_SERVER['SCRIPT_FILENAME']))))).'/public/files/'.date("YmdH",time()).'/',
            'upload_url' => $this->get_full_url().'/public/files/'.date("YmdH",time()).'/',
            'user_dirs' => false,
            'mkdir_mode' => 0755,
            'param_name' => 'files',
    ...
    protected function get_full_url() {
        ...
        return
            ...
            substr(dirname(dirname(dirname(dirname($_SERVER['SCRIPT_NAME'])))),0, strrpos(dirname(dirname(dirname(dirname($_SERVER['SCRIPT_NAME'])))), '/'));
    }
    ...
    protected function get_file_object($file_name) {
        if ($this->is_valid_file_object($file_name)) {
			...
			$file->path = '/public/files/'.date("YmdH",time()).'/'.$file_name;//^_^......
    ...
    protected function handle_file_upload($uploaded_file, $name, $size, $type, $error,
        ...
        if ($this->validate($uploaded_file, $file, $error, $index)) {
			...
			$file->path = '/public/files/'.date("YmdH",time()).'/'.$file->name;//^_^......
            $file->size = $file_size;
            $this->set_file_delete_properties($file);
        }
        return $file;
