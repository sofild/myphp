<?php
	class uploadfile{
		function upload($filedata,$savepath,$returnpath='',$tag=1){
			$inputName = $filedata;//表单文件域name
			$attachDir=$savepath;//上传文件保存路径，结尾不要带/
			...
			if($err==''){
				...
				if(preg_match('/^('.str_replace(',','|',$upExt).')$/i',$extension))
				{
					...
					else
					{
						...
						$targetPath=$this->jsonString($returnpath);
