...
class Page3 {
    ...
    public function show($cate,$ajax=0) {
        ...
        if ($upRow>0){
			if($ajax==0){	
            	$upPage="<a href='#b' onclick='chpage(".$cate.",".$upRow.");' class='pgbtn'>".$this->config['prev']."</a>";
			}
			else{
        ...
        if ($downRow <= $this->totalPages){
			if($ajax==0){
            	$downPage="<a id='page' href='#b' onclick='chpage(".$cate.",".$downRow.");' class='pgbtn'>".$this->config['next']."</a>";
			}
			else{
        ...
        if($nowCoolPage == 1){
			...
			if($ajax==0){
            	$prePage = "<a href='#b' onclick='chpage(".$cate.",".$preRow.");' >prev ".$this->rollPage."</a>";
			}
			else{
        ...
        if($nowCoolPage == $this->coolPages){
            ...
            if($ajax==0){
				$nextPage = "<a id='page' href='#b' onclick='chpage(".$cate.",".$nextRow.");' >next".$this->rollPage."</a>";
			}
			else{
        ...
        if($this->totalPages <=7)
        {
		      for($i=1;$i<=$this->totalPages;$i++)
		      {
						...
						if($page!=$this->nowPage)
						{
							...
							{
								if($ajax==0){
									$linkPage .= "<a href='#b' onclick='chpage(".$cate.",".$page.");'>".$page."</a>";
								}
								else{
		...
		else
		{
			if($this->nowPage - 2 >1)
			{
				if($this->totalPages - $this->nowPage >= 4)
				{
				  ...
				  {
					  if($ajax==0){
					   	$linkPage .= "<a href='#b' onclick='chpage(".$cate.",".$k.");'>".$k."</a>";
					  }
					  else{
			...
			if($kkdc== $this->nowPage){
				...
				if($ajax==0){
					$linkPage .= "<a href='#b' onclick='chpage(".$cate.",".$kkdc.");'>".$kkdc."</a>";
				}
				else{
			...
			if($this->totalPages== $this->nowPage){
				...
				if($ajax==0){
					$linkPage .= "<a href='#b' onclick='chpage(".$cate.",".$this->totalPages.");'>".$this->totalPages."</a>";
				}
				else{
