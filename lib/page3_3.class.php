...
class Page3 {
    ...
    protected $config  =	array('header'=>'条记录','prev'=>'上一页','next'=>'下一页','first'=>'first Page','last'=>'last Page','theme'=>'%upPage%%linkPage%%downPage%');
    // 默认分页变量名
    protected $varPage;
     ...
     * @access public
     +----------------------------------------------------------
     */
    public function show($d1,$d2='',$d3='',$d4='',$d5='',$d6='',$d7='',$d8='',$ajax=0) {
        ...
        if ($upRow>0){
			if($ajax==0){	
            	$upPage="<a href='#b' onclick='chpage2(".$d1.",".$d2.",".$d3.",".$d4.",".$d5.",".$d6.",".$d7.",".$d8.",".$upRow.");' class='pgbtn'>".$this->config['prev']."</a>";
			}
			else{
        ...
        if ($downRow <= $this->totalPages){
			if($ajax==0){
            	$downPage="<a id='page' href='#b' onclick='chpage2(".$d1.",".$d2.",".$d3.",".$d4.",".$d5.",".$d6.",".$d7.",".$d8.",".$upRow.");' class='pgbtn'>".$this->config['next']."</a>";
			}
			else{
        ...
        if($nowCoolPage == 1){
			...
			if($ajax==0){
            	$prePage = "<a href='#b' onclick='chpage2(".$d1.",".$d2.",".$d3.",".$d4.",".$d5.",".$d6.",".$d7.",".$d8.",".$upRow.");' >prev ".$this->rollPage."</a>";
			}
			else{
        ...
        if($nowCoolPage == $this->coolPages){
            ...
            if($ajax==0){
				$nextPage = "<a id='page' href='#b' onclick='chpage2(".$d1.",".$d2.",".$d3.",".$d4.",".$d5.",".$d6.",".$d7.",".$d8.",".$upRow.");' >next".$this->rollPage."</a>";
			}
			else{
        ...
        if($this->totalPages <=7)
        {
		      for($i=1;$i<=$this->totalPages;$i++)
		      {
						...
						if($page!=$this->nowPage)
						{
							...
							{
								if($ajax==0){
									$linkPage .= "<a href='#b' onclick='chpage2(".$d1.",".$d2.",".$d3.",".$d4.",".$d5.",".$d6.",".$d7.",".$d8.",".$upRow.");'>".$page."</a>";
								}
								else{
		...
		else
		{
			if($this->nowPage - 2 >1)
			{
				if($this->totalPages - $this->nowPage >= 4)
				{
				  ...
				  {
					  if($ajax==0){
					   	$linkPage .= "<a href='#b' onclick='chpage2(".$d1.",".$d2.",".$d3.",".$d4.",".$d5.",".$d6.",".$d7.",".$d8.",".$upRow.");'>".$k."</a>";
					  }
					  else{
			...
			if($kkdc== $this->nowPage){
				...
				if($ajax==0){
					$linkPage .= "<a href='#b' onclick='chpage2(".$d1.",".$d2.",".$d3.",".$d4.",".$d5.",".$d6.",".$d7.",".$d8.",".$upRow.");'>".$kkdc."</a>";
				}
				else{
			...
			if($this->totalPages== $this->nowPage){
				...
				if($ajax==0){
					$linkPage .= "<a href='#b' onclick='chpage2(".$d1.",".$d2.",".$d3.",".$d4.",".$d5.",".$d6.",".$d7.",".$d8.",".$upRow.");'>".$this->totalPages."</a>";
				}
				else{
